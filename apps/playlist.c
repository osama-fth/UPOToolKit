...
int by_album_comparator(const void *a, const void *b)
{
    ...
    return strcmp(entry_a->album, entry_b->album);
}
...
int by_year_comparator(const void *a, const void *b)
{
    ...
    return entry_a->year - entry_b->year;
}
...
int by_track_number_comparator(const void *a, const void *b)
{
    ...
    return entry_a->track_num - entry_b->track_num;
}
...
int by_track_title_comparator(const void *a, const void *b)
{
    ...
    return strcmp(entry_a->track_title, entry_b->track_title);
}
...
void playlist_sort(playlist_t playlist, playlist_sorting_criterion_t order_by)
{
   if (playlist == NULL || playlist->size == 0) 
    ...
    upo_sort_comparator_t cmp = NULL;
    
    switch (order_by) {
        case playlist_by_artist_sorting_criterion:
            cmp = (upo_sort_comparator_t)by_artist_comparator;
            break;
        case playlist_by_album_sorting_criterion:
            cmp = (upo_sort_comparator_t)by_album_comparator;
            break;
        case playlist_by_year_sorting_criterion:
            cmp = (upo_sort_comparator_t)by_year_comparator;
            break;
        case playlist_by_track_number_sorting_criterion:
            cmp = (upo_sort_comparator_t)by_track_number_comparator;
            break;
        case playlist_by_track_title_sorting_criterion:
            cmp = (upo_sort_comparator_t)by_track_title_comparator;
            break;
        default:
            return;
    }

    // upo_insertion_sort(playlist->entries, playlist->size, sizeof(entry_t), cmp);
    // upo_merge_sort(playlist->entries, playlist->size, sizeof(entry_t), cmp);
